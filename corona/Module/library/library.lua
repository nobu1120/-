...
function checkDownload2(url, action, subDir, dir, root)
	...
	if startsWith(url, "http://") then
		...
		elseif isUrl then
			...
			local params = {}
			params.headers = headers
			network.download(url, "GET", listener, filePath..subDirectory.."/"..basename(url), directory)
...
local function setAction(action, group, parameter)
	...
	-- ツイッターアカウントをフォロー促し
	if action == "tw_follow" then
		local resPopup = require( libDir .. "responsePopup" )
        local tsTwitter = require(libDir.."tsTwitter")
        local twitter = tsTwitter.new()
        twitter.follow(
            resPopup.new({type=44, text='フォローしました'})
        )		
	end

	if action == 'point' then
		if parameter.point and  userInfoData then
			userInfoData['point'] = tonumber(parameter.point)
			setUserInfo.reload()

			local leftMenu = require( libDir .. 'leftMenu' )
			leftMenu.changeInfo()
		end
	end

	-- レスポンスのポップアップ
	if action == "res_popup" then
		local resPopup = require( libDir .. "responsePopup" )
		resPopup.new( { type = 44, text = parameter.text } )
	end

	-- レスポンスのポップアップ(カスタマイズ)
	if action == "cus_popup" then
		local resPopup = require( libDir .. "responsePopup" )
		resPopup.new( { type = 0, image = parameter.image, text = parameter.text } )
	end


	-- ツイッター連携
	if action == "tw_connect" then
		local function listener()
			local resPopup = require( libDir .. "responsePopup" )
			resPopup.new({type=36})
		end
		local share = require(libDir.."share_model")
		share.connect({type="twitter", endAction=listener})
	end

	-- ツイッター投稿
	if action == "tw_post" then
		local function listener()
			local resPopup = require( libDir .. "responsePopup" )
			resPopup.new({type=18})
		end		
		local share = require(libDir.."share_model")
		if share.connectCheck("twitter") then
			share.post({type="twitter", twitter=true, type='popup' ,endAction=listener, comment=parameter.message})
		else
			share.connect({type="twitter", endAction=
				function()
					share.post({type="twitter", twitter=true, type='popup' ,endAction=listener, comment=parameter.message})
				end
			})
	...
	-- facebook 連携
	if action == "fb_connect" then
		local function listener()
			local resPopup = require( libDir .. "responsePopup" )
			resPopup.new({type=37})
		end		
		local share = require(libDir.."share_model")
		share.connect({type="facebook", endAction=listener})
	end

	if action == "set_mic" then
		local resPopup = require( libDir .. "responsePopup" )
		local recButton = require(libDir .. "recButton")

		local function func()
			-- マイクのデータ取得
			recButton.getMicData({endAction = 
				function()
					resPopup.new({type=10})
				end
			})
		end
		return func
	...
	-- スタンプのセット
	if action == "set_stamp" then
		local stampArea = require(libDir.."stampArea")
		stampArea.genreDownload()
	end

	if action == "page" then
	 -- local function func()
			tsGotoScene(parameter)
			display.remove(group)
			group=nil   
			stopAllPlayer()
	...
	if action == "stampDL" then
		local function func()
			...
			storyboard.gotoScene( "present", "slideLeft",400)
		end
		return func
	end
	...
	if action == "request" then
		...
		local function listener(event)
			...
			if parameter.resPopup then
				local resPopup = require(libDir.."responsePopup")
				resPopup.new({type=parameter.resPopup})
			end
		...
		loadingModal(_W*0.5,_H*0.5,nil,0,-5,_W, _H,false,1)
	end
...
function ResourceManeger(data, endAction)
	...
	local function removeAll() 
		...
		native.cancelWebPopup()
		stopAllPlayer()
		tsanalysis.event({name="E002", status1=_deviceID, status2=popupID  ,status3=os.difftime(os.time(), popupShowTime)})
...
function showRemotePopup(url)
	...
	local MultipartFormData = require("class_MultipartFormDataBinary") 
	local multipart = MultipartFormData.new()
...
function convertNumberToImage( str )

	local group = display.newGroup()
	local str = tostring(str)

	for i=1 , str:len() do
		local num = display.newImage( group , ImgDir .. "number/" .. string.sub(str,i,i) .. ".png" , 0 , 0 )
		num.x = 50*i
		num.y = 0
	end
